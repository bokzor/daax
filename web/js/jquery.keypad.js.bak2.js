...
&lt;div&gt;&lt;/div&gt;</pre>
	...
	return ch.toUpperCase();
            ...
            if (!inst._inline) {
                ...
                if (inst.options.showOn == 'button' || inst.options.showOn == 'both') {
                    ...
                    var trigger = $(inst.options.buttonImageOnly ?
                        ...
                        $('<button type="button" title="' + buttonStatus + '"></button>').html(buttonImage == '' ? inst.options.buttonText :
                            $('<img src="' + buttonImage + '" alt="' +
                                buttonStatus + '" title="' + buttonStatus + '"/>')));
        ...
        _preDestroy: function(elem, inst) {
            ...
            elem.empty().off('.' + inst.name)[inst.saveReadonly ? 'attr' : 'removeAttr']('readonly', true);
        ...
        enable: function(elem) {
            ...
            if (nodeName.match(/input|textarea/)) {
                ...
                siblings('img.' + this._triggerClass).css({
                    opacity: '1.0',
                    cursor: ''
                });
        ...
        _updateKeypad: function(inst) {
            ...
            if ($.isFunction(inst.options.beforeShow)) {
                inst.options.beforeShow.apply((inst._input ? inst._input[0] : null), [inst._mainDiv, inst]);
            }
        },
        ...
        _checkExternalClick: function(event) {
            ...
            if (target.closest('.' + plugin._mainDivClass).length === 0 && !target.hasClass(plugin._getMarker()) &&
        ...
        _notifyKeypress: function(inst, key) {
            if ($.isFunction(inst.options.onKeypress)) { // trigger custom callback
                inst.options.onKeypress.apply((inst._input ? inst._input[0] : null), [key, inst._input.val(), inst]);
            }
        },
