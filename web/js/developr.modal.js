...
(function($, window, document) {
    ...
    $.modal = function(options) {
        ...
        $.each(settings.buttons, function(name, config) {
            ...
            // Insert
            $('<button type="button" class="button huge' + (config.classes ? ' ' + config.classes : '') + spacingClass + '">' + name + '</button>').appendTo(buttonsBlock).click(function(event) {
                config.click.call(this, $(this).closest('.modal'), event);
            });
        });
    ...
    $.modal.prompt = function(message, callback, cancelCallback, options) {
        ...
        // Content
        options.content = '<div class="margin-bottom">' + message + '</div><div class="input full-width"><input type="number" step="1" name="prompt-value" id="prompt-value" value="" class="input-unstyled full-width"></div><script> if (!$.template.touchOs){$(\'#prompt-value\').keypad(); $(\'#prompt-value\').keypad(\'show\')} else{  document.getElementById(\'prompt-value\').focus(); } </script>';
    ...
    $.modal.confirm = function(message, confirmCallback, cancelCallback, options) {
        ...
        options.buttons[options.textConfirm] = {
            classes: 'blue-gradient glossy huge',
            click: function(modal) {
                // Mark as sumbmitted to prevent the cancel callback to fire
                isConfirmed = true;
    ...
    $.fn.modal = function(options) {
        ...
        this.each(function() {
            var element = $(this);
            modals.add($.modal($.extend({}, options, element.data('modal-options'), {
                content: element
            })));
        });
