...
function Keypad() {
	...
	this.regional[''] = { // Default regional settings
		...
		backText: 'Retour', // Display text for back link
...
$.extend(Keypad.prototype, {
	...
	_showPlugin: function(field) {
		...
		inst._mainDiv.css({position: 'absolute', display: 'block', width: 'auto'});
		plugin._updateKeypad(inst);
		// and adjust position before showing
		offset = plugin._checkOffset(inst, offset, isFixed);
		inst._mainDiv.css({position: (isFixed ? 'fixed' : 'absolute'), display: 'none',
			left: 0, right: 0, margin: 'auto', bottom: 0 + 'px'});
	...
	_checkOffset: function(inst, offset, isFixed) {
		...
		if ((offset.top + inst._mainDiv.outerHeight() - scrollY) > browserHeight) {
			...
			offset.top = Math.max((isFixed ? 0 : scrollY), 100 + offset.top - (isFixed ? scrollY : 0));
