        ...
        if (data['total_commande'] == 1) {
            ...
            $.each(data['articles'], function (key, val) {
                for (i = 0; i < val['count']; i++) {
                    var boisson = { 'prix': val['prix'], 'name': val['name'], 'id': val['id'], 'supplements': val['supplements'], 'comment': val['comment'] };
                    addBoisson( boisson );
                }
            });
        ...
        if ($('#supplementB').hasClass('red-gradient')) {
            var buttons = {
                ...
                'Valider': function (modal) {
                    ...
                    for (i = 0; i < count; i++) {
                        var boisson = { 'prix': price, 'name': name, 'id': idBoisson, 'supplements': supplements }
                        addBoisson( boisson );
                    }
                    modal.closeModal();
                },
            ...
            var boisson = { 'prix': price, 'name': name, 'id': idBoisson, 'supplements': undefined }
            id = addBoisson(boisson);
            boisson = app.collections.commande.findWhere({id_article: id});
...
function chargerPage(page) {
    ...
    $('#main').load(url, function () {
        $('#load').transition({ opacity: 0}, 100, function () {
