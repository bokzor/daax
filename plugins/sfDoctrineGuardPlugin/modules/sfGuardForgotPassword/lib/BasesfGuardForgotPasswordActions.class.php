    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        $message = Swift_Message::newInstance()
          ->setFrom(sfConfig::get('app_sf_guard_plugin_default_from_email', 'admin@ergor.org'))
          ->setTo($this->form->user->email_address)
          ->setSubject('Demande de changement de mot de passe | Ergor.org')
        ...
        $this->getUser()->setFlash('notice', 'Vérifiez vos mails. Vous devriez bientôt recevoir quelque chose !');
        $this->redirect('@homepage');
      } else {
        $this->getUser()->setFlash('error', 'Email invalide!');
      }
    }
  }
  ...
  public function executeChange($request)
  {
    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        $this->getUser()->setFlash('notice', 'Votre mot de passe a été changé !');
        $this->redirect('@homepage');
      }
    }
  }
